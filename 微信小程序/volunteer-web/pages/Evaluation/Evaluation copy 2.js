// import subjectdata from '../../Json/subject.json';

Page({

  /**
   * 页面的初始数据
   */
  data: {
    radio: '',
    LeftValue: '精简版',
    RightValue: '详细版',
    // 每个答案
    EachAnswer: '',
    Answer: [],
    //题目新
    AnswerArr: [
      {
        "subjectTopic": "当你要外出一整天，你会",
        "answer": {
          "A": "计划你要做什么和在什么时候做",
          "B": "说去就去",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "你认为自己是一个",
        "answer": {
          "A": "较为随兴所至的人",
          "B": "较为有条理的人",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "假如你是一位老师，你会选教",
        "answer": {
          "A": "以事实为主的课程",
          "B": "涉及理论的课程",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你通常",
        "answer": {
          "A": "与人容易混熟",
          "B": "比较沉静或矜持",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "一般来说，你和哪些人比较合得来？",
        "answer": {
          "A": "富于想象力的人",
          "B": "现实的人",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你是否经常让",
        "answer": {
          "A": "你的情感支配你的理智",
          "B": "你的理智主宰你的情感",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "处理许多事情上，你会喜欢",
        "answer": {
          "A": "凭兴所至行事",
          "B": "按照计划行事",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "你是否",
        "answer": {
          "A": "容易让人了解",
          "B": "难于让人了解",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "按照程序表做事",
        "answer": {
          "A": "合你心意",
          "B": "令你感到束缚",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "当你有一份特别的任务，你会喜欢",
        "answer": {
          "A": "开始前小心组织计划",
          "B": "边做边找须做什么",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "在大多数情况下，你会选择",
        "answer": {
          "A": "顺其自然",
          "B": "按程序表做事",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "大多数人会说你是一个",
        "answer": {
          "A": "重视自我隐私的人",
          "B": "非常坦率开放的人",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你宁愿被人认为是一个",
        "answer": {
          "A": "实事求是的人",
          "B": "机灵的人",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在一大群人当中，通常是",
        "answer": {
          "A": "你介绍大家认识",
          "B": "别人介绍你",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你会跟哪些人做朋友？",
        "answer": {
          "A": "常提出新注意的",
          "B": "脚踏实地的",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你倾向",
        "answer": {
          "A": "重视感情多于逻辑",
          "B": "重视逻辑多于感情",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你比较喜欢",
        "answer": {
          "A": "坐观事情发展才作计划",
          "B": "很早就作计划",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "你喜欢花很多的时间",
        "answer": {
          "A": "一个人独处",
          "B": "和别人在一起",
          "E": "E",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "与很多人一起会",
        "answer": {
          "A": "令你活力培增",
          "B": "常常令你心力憔悴",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你比较喜欢",
        "answer": {
          "A": "很早便把约会、社交聚集等事情安排妥当",
          "B": "无拘无束，看当时有什么好玩就做什么",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "计划一个旅程时，你较喜欢",
        "answer": {
          "A": "大部分的时间都是跟当天的感觉行事",
          "B": "事先知道大部分的日子会做什么",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "在社交聚会中，你",
        "answer": {
          "A": "有时感到郁闷",
          "B": "常常乐在其中",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你通常",
        "answer": {
          "A": "和别人容易混熟",
          "B": "趋向自处一隅",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "哪些人会更吸引你？",
        "answer": {
          "A": "一个思想敏捷及非常聪颖的人",
          "B": "实事求是，具丰富常识的人",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在日常工作中，你会",
        "answer": {
          "A": "颇为喜欢处理迫使你分秒必争的突发",
          "B": "通常预先计划，以免要在压力下工作",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "你认为别人一般",
        "answer": {
          "A": "要花很长时间才认识你",
          "B": "用很短的时间便认识你",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "注重隐私",
          "B": "坦率开放",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "预先安排的",
          "B": "无计划的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "抽象",
          "B": "具体",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "温柔",
          "B": "坚定",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "思考",
          "B": "感受",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "事实",
          "B": "意念",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "冲动",
          "B": "决定",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "热衷",
          "B": "文静",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "文静",
          "B": "外向",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "有系统",
          "B": "随意",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "理论",
          "B": "肯定",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "敏感",
          "B": "公正",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "令人信服",
          "B": "感人的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "声明",
          "B": "概念",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "不受约束",
          "B": "预先安排",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "J",
          "P": "A"
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "矜持",
          "B": "健谈",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "有条不紊",
          "B": "不拘小节",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "意念",
          "B": "实况",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "同情怜悯",
          "B": "远见",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "利益",
          "B": "祝福",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "务实的",
          "B": "理论的",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "朋友不多",
          "B": "朋友众多",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "有系统",
          "B": "即兴",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "富想象的",
          "B": "以事论事",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "亲切的",
          "B": "客观的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "客观的",
          "B": "热情的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "建造",
          "B": "发明",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "文静",
          "B": "爱合群",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "理论",
          "B": "事实",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "富同情",
          "B": "合逻辑",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "具分析力",
          "B": "多愁善感",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "合情合理",
          "B": "令人着迷",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "当你要在一个星期内完成一个大项目，你在开始的时候会",
        "answer": {
          "A": "把要做的不同工作依次列出",
          "B": "马上动工",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "P"
        }
      },
      {
        "subjectTopic": "在社交场合中，你经常会感到",
        "answer": {
          "A": "与某些人很难打开话匣儿和保持对话",
          "B": "与多数人都能从容地长谈",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "要做许多人也做的事，你比较喜欢",
        "answer": {
          "A": "按照一般认可的方法去做",
          "B": "构想一个自己的想法",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你刚认识的朋友能否说出你的兴趣？",
        "answer": {
          "A": "马上可以",
          "B": "要待他们真正了解你之后才可以",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你通常较喜欢的科目是",
        "answer": {
          "A": "讲授概念和原则的",
          "B": "讲授事实和数据的",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "哪个是较高的赞誉，或称许为",
        "answer": {
          "A": "一贯感性的人",
          "B": "一贯理性的人",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你认为按照程序表做事",
        "answer": {
          "A": "有时是需要的，但一般来说你不大喜欢这样做",
          "B": "大多数情况下是有帮助而且是你喜欢做的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "和一群人在一起，你通常会选",
        "answer": {
          "A": "跟你很熟悉的个别人谈话",
          "B": "参与大伙的谈话",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在社交聚会上，你会",
        "answer": {
          "A": "是说话很多的一个",
          "B": "让别人多说话",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "把周末期间要完成的事列成清单，这个主意会",
        "answer": {
          "A": "合你意",
          "B": "使你提不起劲",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "哪个是较高的赞誉，或称许为",
        "answer": {
          "A": "能干的",
          "B": "富有同情心",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你通常喜欢",
        "answer": {
          "A": "事先安排你的社交约会",
          "B": "随兴之所至做事",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "A",
          "P": "B"
        }
      },
      {
        "subjectTopic": "总的说来，要做一个大型作业时，你会选",
        "answer": {
          "A": "边做边想该做什么",
          "B": "首先把工作按步细分",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "你能否滔滔不绝地与人聊天",
        "answer": {
          "A": "只限于跟你有共同兴趣的人",
          "B": "几乎跟任何人都可以",
          "E": "B",
          "I": "A",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你会",
        "answer": {
          "A": "跟随一些证明有效的方法",
          "B": "分析还有什么毛病，及针对尚未解决的难题",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "为乐趣而阅读时，你会",
        "answer": {
          "A": "喜欢奇特或创新的表达方式",
          "B": "喜欢作者直话直说",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你宁愿替哪一类上司（或者老师）工作？",
        "answer": {
          "A": "天性淳良，但常常前后不一的",
          "B": "言词尖锐但永远合乎逻辑的",
          "E": "",
          "I": "",
          "S": "",
          "N": "B",
          "T": "A",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "你做事多数是",
        "answer": {
          "A": "按当天心情去做",
          "B": "照拟好的程序表去做",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "B",
          "P": "A"
        }
      },
      {
        "subjectTopic": "你是否",
        "answer": {
          "A": "可以和任何人按需求从容地交谈",
          "B": "只是对某些人或在某种情况下才可以畅所欲言",
          "E": "A",
          "I": "B",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "要作决定时，你认为比较重要的是",
        "answer": {
          "A": "据事实衡量",
          "B": "考虑他人的感受和意见",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "想象的",
          "B": "真实的",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "仁慈慷慨的",
          "B": "意志坚定的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "公正的",
          "B": "有关怀心",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "制作",
          "B": "设计",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "可能性",
          "B": "必然性",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "温柔",
          "B": "力量",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "实际",
          "B": "多愁善感",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "制造",
          "B": "创造",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "新颖的",
          "B": "已知的",
          "E": "",
          "I": "",
          "S": "B",
          "N": "A",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "同情",
          "B": "分析",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "坚持己见",
          "B": "温柔有爱心",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "具体的",
          "B": "抽象的",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "全心投入",
          "B": "有决心的",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "B",
          "F": "A",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "能干",
          "B": "仁慈",
          "E": "",
          "I": "",
          "S": "",
          "N": "",
          "T": "A",
          "F": "B",
          "J": "",
          "P": ""
        }
      },
      {
        "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
        "answer": {
          "A": "实际",
          "B": "创新",
          "E": "",
          "I": "",
          "S": "A",
          "N": "B",
          "T": "",
          "F": "",
          "J": "",
          "P": ""
        }
      }
    ],
    //题目旧
    // AnswerArr: [{
    //   "Topic1": {
    //     "Topic": "一.哪一个答案最能贴切的描绘你一般的感受或行为？",
    //     "subjectTopic1": {
    //       "subjectTopic": "当你要外出一整天，你会",
    //       "answer": {
    //         "A": "计划你要做什么和在什么时候做",
    //         "B": "说去就去",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic2": {
    //       "subjectTopic": "你认为自己是一个",
    //       "answer": {
    //         "A": "较为随兴所至的人",
    //         "B": "较为有条理的人",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic3": {
    //       "subjectTopic": "假如你是一位老师，你会选教",
    //       "answer": {
    //         "A": "以事实为主的课程",
    //         "B": "涉及理论的课程",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic4": {
    //       "subjectTopic": "你通常",
    //       "answer": {
    //         "A": "与人容易混熟",
    //         "B": "比较沉静或矜持",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic5": {
    //       "subjectTopic": "一般来说，你和哪些人比较合得来？",
    //       "answer": {
    //         "A": "富于想象力的人",
    //         "B": "现实的人",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic6": {
    //       "subjectTopic": "你是否经常让",
    //       "answer": {
    //         "A": "你的情感支配你的理智",
    //         "B": "你的理智主宰你的情感",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic7": {
    //       "subjectTopic": "处理许多事情上，你会喜欢",
    //       "answer": {
    //         "A": "凭兴所至行事",
    //         "B": "按照计划行事",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic8": {
    //       "subjectTopic": "你是否",
    //       "answer": {
    //         "A": "容易让人了解",
    //         "B": "难于让人了解",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic9": {
    //       "subjectTopic": "按照程序表做事",
    //       "answer": {
    //         "A": "合你心意",
    //         "B": "令你感到束缚",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic10": {
    //       "subjectTopic": "当你有一份特别的任务，你会喜欢",
    //       "answer": {
    //         "A": "开始前小心组织计划",
    //         "B": "边做边找须做什么",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic11": {
    //       "subjectTopic": "在大多数情况下，你会选择",
    //       "answer": {
    //         "A": "顺其自然",
    //         "B": "按程序表做事",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic12": {
    //       "subjectTopic": "大多数人会说你是一个",
    //       "answer": {
    //         "A": "重视自我隐私的人",
    //         "B": "非常坦率开放的人",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic13": {
    //       "subjectTopic": "你宁愿被人认为是一个",
    //       "answer": {
    //         "A": "实事求是的人",
    //         "B": "机灵的人",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic14": {
    //       "subjectTopic": "在一大群人当中，通常是",
    //       "answer": {
    //         "A": "你介绍大家认识",
    //         "B": "别人介绍你",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic15": {
    //       "subjectTopic": "你会跟哪些人做朋友？",
    //       "answer": {
    //         "A": "常提出新注意的",
    //         "B": "脚踏实地的",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic16": {
    //       "subjectTopic": "你倾向",
    //       "answer": {
    //         "A": "重视感情多于逻辑",
    //         "B": "重视逻辑多于感情",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic17": {
    //       "subjectTopic": "你比较喜欢",
    //       "answer": {
    //         "A": "坐观事情发展才作计划",
    //         "B": "很早就作计划",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic18": {
    //       "subjectTopic": "你喜欢花很多的时间",
    //       "answer": {
    //         "A": "一个人独处",
    //         "B": "和别人在一起",
    //         "E": "E",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic19": {
    //       "subjectTopic": "与很多人一起会",
    //       "answer": {
    //         "A": "令你活力培增",
    //         "B": "常常令你心力憔悴",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic20": {
    //       "subjectTopic": "你比较喜欢",
    //       "answer": {
    //         "A": "很早便把约会、社交聚集等事情安排妥当",
    //         "B": "无拘无束，看当时有什么好玩就做什么",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic21": {
    //       "subjectTopic": "计划一个旅程时，你较喜欢",
    //       "answer": {
    //         "A": "大部分的时间都是跟当天的感觉行事",
    //         "B": "事先知道大部分的日子会做什么",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic22": {
    //       "subjectTopic": "在社交聚会中，你",
    //       "answer": {
    //         "A": "有时感到郁闷",
    //         "B": "常常乐在其中",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic23": {
    //       "subjectTopic": "你通常",
    //       "answer": {
    //         "A": "和别人容易混熟",
    //         "B": "趋向自处一隅",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic24": {
    //       "subjectTopic": "哪些人会更吸引你？",
    //       "answer": {
    //         "A": "一个思想敏捷及非常聪颖的人",
    //         "B": "实事求是，具丰富常识的人",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic25": {
    //       "subjectTopic": "在日常工作中，你会",
    //       "answer": {
    //         "A": "颇为喜欢处理迫使你分秒必争的突发",
    //         "B": "通常预先计划，以免要在压力下工作",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic26": {
    //       "subjectTopic": "你认为别人一般",
    //       "answer": {
    //         "A": "要花很长时间才认识你",
    //         "B": "用很短的时间便认识你",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     }
    //   },
    //   "Topic2": {
    //     "Topic": "二.在下列每一对词语中，哪一个词语更合你心意？",
    //     "subjectTopic27": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "注重隐私",
    //         "B": "坦率开放",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic28": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "预先安排的",
    //         "B": "无计划的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic29": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "抽象",
    //         "B": "具体",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic30": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "温柔",
    //         "B": "坚定",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic31": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "思考",
    //         "B": "感受",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic32": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "事实",
    //         "B": "意念",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic33": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "冲动",
    //         "B": "决定",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic34": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "热衷",
    //         "B": "文静",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic35": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "文静",
    //         "B": "外向",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic36": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "有系统",
    //         "B": "随意",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic37": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "理论",
    //         "B": "肯定",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic38": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "敏感",
    //         "B": "公正",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic39": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "令人信服",
    //         "B": "感人的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic40": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "声明",
    //         "B": "概念",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic41": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "不受约束",
    //         "B": "预先安排",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "J",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic42": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "矜持",
    //         "B": "健谈",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic43": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "有条不紊",
    //         "B": "不拘小节",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic44": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "意念",
    //         "B": "实况",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic45": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "同情怜悯",
    //         "B": "远见",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic46": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "利益",
    //         "B": "祝福",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic47": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "务实的",
    //         "B": "理论的",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic48": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "朋友不多",
    //         "B": "朋友众多",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic49": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "有系统",
    //         "B": "即兴",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic50": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "富想象的",
    //         "B": "以事论事",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic51": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "亲切的",
    //         "B": "客观的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic52": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "客观的",
    //         "B": "热情的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic53": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "建造",
    //         "B": "发明",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic54": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "文静",
    //         "B": "爱合群",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic55": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "理论",
    //         "B": "事实",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic56": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "富同情",
    //         "B": "合逻辑",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic57": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "具分析力",
    //         "B": "多愁善感",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic58": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "合情合理",
    //         "B": "令人着迷",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     }
    //   },
    //   "Topic3": {
    //     "Topic": "三.哪一个答案最能贴切地描绘你一般的感受或行为",
    //     "subjectTopic59": {
    //       "subjectTopic": "当你要在一个星期内完成一个大项目，你在开始的时候会",
    //       "answer": {
    //         "A": "把要做的不同工作依次列出",
    //         "B": "马上动工",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "P"
    //       }
    //     },
    //     "subjectTopic60": {
    //       "subjectTopic": "在社交场合中，你经常会感到",
    //       "answer": {
    //         "A": "与某些人很难打开话匣儿和保持对话",
    //         "B": "与多数人都能从容地长谈",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic61": {
    //       "subjectTopic": "要做许多人也做的事，你比较喜欢",
    //       "answer": {
    //         "A": "按照一般认可的方法去做",
    //         "B": "构想一个自己的想法",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic62": {
    //       "subjectTopic": "你刚认识的朋友能否说出你的兴趣？",
    //       "answer": {
    //         "A": "马上可以",
    //         "B": "要待他们真正了解你之后才可以",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic63": {
    //       "subjectTopic": "你通常较喜欢的科目是",
    //       "answer": {
    //         "A": "讲授概念和原则的",
    //         "B": "讲授事实和数据的",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic64": {
    //       "subjectTopic": "哪个是较高的赞誉，或称许为",
    //       "answer": {
    //         "A": "一贯感性的人",
    //         "B": "一贯理性的人",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic65": {
    //       "subjectTopic": "你认为按照程序表做事",
    //       "answer": {
    //         "A": "有时是需要的，但一般来说你不大喜欢这样做",
    //         "B": "大多数情况下是有帮助而且是你喜欢做的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic66": {
    //       "subjectTopic": "和一群人在一起，你通常会选",
    //       "answer": {
    //         "A": "跟你很熟悉的个别人谈话",
    //         "B": "参与大伙的谈话",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic67": {
    //       "subjectTopic": "在社交聚会上，你会",
    //       "answer": {
    //         "A": "是说话很多的一个",
    //         "B": "让别人多说话",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic68": {
    //       "subjectTopic": "把周末期间要完成的事列成清单，这个主意会",
    //       "answer": {
    //         "A": "合你意",
    //         "B": "使你提不起劲",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic69": {
    //       "subjectTopic": "哪个是较高的赞誉，或称许为",
    //       "answer": {
    //         "A": "能干的",
    //         "B": "富有同情心",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic70": {
    //       "subjectTopic": "你通常喜欢",
    //       "answer": {
    //         "A": "事先安排你的社交约会",
    //         "B": "随兴之所至做事",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "A",
    //         "P": "B"
    //       }
    //     },
    //     "subjectTopic71": {
    //       "subjectTopic": "总的说来，要做一个大型作业时，你会选",
    //       "answer": {
    //         "A": "边做边想该做什么",
    //         "B": "首先把工作按步细分",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic72": {
    //       "subjectTopic": "你能否滔滔不绝地与人聊天",
    //       "answer": {
    //         "A": "只限于跟你有共同兴趣的人",
    //         "B": "几乎跟任何人都可以",
    //         "E": "B",
    //         "I": "A",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic73": {
    //       "subjectTopic": "你会",
    //       "answer": {
    //         "A": "跟随一些证明有效的方法",
    //         "B": "分析还有什么毛病，及针对尚未解决的难题",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic74": {
    //       "subjectTopic": "为乐趣而阅读时，你会",
    //       "answer": {
    //         "A": "喜欢奇特或创新的表达方式",
    //         "B": "喜欢作者直话直说",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic75": {
    //       "subjectTopic": "你宁愿替哪一类上司（或者老师）工作？",
    //       "answer": {
    //         "A": "天性淳良，但常常前后不一的",
    //         "B": "言词尖锐但永远合乎逻辑的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "B",
    //         "T": "A",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic76": {
    //       "subjectTopic": "你做事多数是",
    //       "answer": {
    //         "A": "按当天心情去做",
    //         "B": "照拟好的程序表去做",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "B",
    //         "P": "A"
    //       }
    //     },
    //     "subjectTopic77": {
    //       "subjectTopic": "你是否",
    //       "answer": {
    //         "A": "可以和任何人按需求从容地交谈",
    //         "B": "只是对某些人或在某种情况下才可以畅所欲言",
    //         "E": "A",
    //         "I": "B",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic78": {
    //       "subjectTopic": "要作决定时，你认为比较重要的是",
    //       "answer": {
    //         "A": "据事实衡量",
    //         "B": "考虑他人的感受和意见",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     }
    //   },
    //   "Topic4": {
    //     "Topic": "四.在下列每一对词语中，哪一个词语更合你心意？",
    //     "subjectTopic79": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "想象的",
    //         "B": "真实的",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic80": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "仁慈慷慨的",
    //         "B": "意志坚定的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic81": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "公正的",
    //         "B": "有关怀心",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic82": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "制作",
    //         "B": "设计",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic83": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "可能性",
    //         "B": "必然性",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic84": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "温柔",
    //         "B": "力量",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic85": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "实际",
    //         "B": "多愁善感",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic86": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "制造",
    //         "B": "创造",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic87": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "新颖的",
    //         "B": "已知的",
    //         "E": "",
    //         "I": "",
    //         "S": "B",
    //         "N": "A",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic88": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "同情",
    //         "B": "分析",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic89": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "坚持己见",
    //         "B": "温柔有爱心",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic90": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "具体的",
    //         "B": "抽象的",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic91": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "全心投入",
    //         "B": "有决心的",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "B",
    //         "F": "A",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic92": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "能干",
    //         "B": "仁慈",
    //         "E": "",
    //         "I": "",
    //         "S": "",
    //         "N": "",
    //         "T": "A",
    //         "F": "B",
    //         "J": "",
    //         "P": ""
    //       }
    //     },
    //     "subjectTopic93": {
    //       "subjectTopic": "在下列每一对词语中，哪一个词语更合你心意？",
    //       "answer": {
    //         "A": "实际",
    //         "B": "创新",
    //         "E": "",
    //         "I": "",
    //         "S": "A",
    //         "N": "B",
    //         "T": "",
    //         "F": "",
    //         "J": "",
    //         "P": ""
    //       }
    //     }
    //   }
    // }],
    AllAnswer: [],//保存答案

  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    let result = [];
    console.log("长度", this.data.AnswerArr.length);
    // if (!this.AnswerArr.length) {
    //   result = [{}];
    //   return false;
    // }
    for (let i = 0; i < this.data.AnswerArr.length; i++) {
      // if (this.questionList[i].type === "radio") {
      //   result.push({ index: i + 1, value: "" });
      // } else {
      //   result.push({ index: i + 1, value: [] });
      // }
      result.push({ index: i + 1, value: "" });
    }
    // console.log("111111111",this.data.Answer);
    this.setData({
      Answer: result
    })
    console.log("111111111",this.data.Answer);
  },

  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function () {
 
  },

  // 答案选择

  onAnswerSelection(event, index) {
    let result = [];
    console.log("长度", this.data.AnswerArr.length);
    // if (!this.AnswerArr.length) {
    //   result = [{}];
    //   return false;
    // }
    for (let i = 0; i < this.data.AnswerArr.length; i++) {
      // if (this.questionList[i].type === "radio") {
      //   result.push({ index: i + 1, value: "" });
      // } else {
      //   result.push({ index: i + 1, value: [] });
      // }
      result.push({ index: i + 1, value: event.detail });
    }
    // console.log("111111111",this.data.Answer);
    this.setData({
      Answer: result
    })
    console.log("111111111",this.data.Answer);


    // console.log("答案选择", event.detail, event.currentTarget.dataset.index, index)
    // // this.data.EachAnswer = EachAnswer[event.currentTarget.dataset.index]

    // this.setData({
    //   Answer: `EachAnswer${event.currentTarget.dataset.index}`,
    //   // Answer: event.detail,
    // });

    // //保存答案
    // this.data.AllAnswer[event.currentTarget.dataset.index] = event.detail
    // console.log(this.data.AllAnswer, this.data.Answer)
  },
  A(index) {
    console.log(index)
    // this.setData({
    //   Answer:"A"
    // })

    //  this.data.Answer[index.currentTarget.dataset.index] = "A"


  },


  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function () {

  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function () {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function () {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function () {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function () {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage: function () {

  }
})